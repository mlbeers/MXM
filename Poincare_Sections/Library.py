...
def poincare_details1(perm, vecs0, generators):
    # Find the eigenvectors for each generator and ensure they are 1
    eigs = []
    for matrix in generators:
        eig1, eig2 = matrix.eigenvalues()
        if eig1 == eig2 == 1:
            ...
            raise ValueError("Eigenvalue not equal to 1 or different eigenvalues")

    # Find the eigenvectors for each generator
    eigenvecs = [np.array([[vec[0]], [vec[1]]]) for matrix in generators for vec in matrix.eigenvectors_right()[0][1]]

    # Find the magnitude, slope, x-direction, and y-direction of each eigenvector
    saddle_vecs = []
    for vec in eigenvecs:
        mag_vec = np.linalg.norm(vec)  # Use numpy's linalg.norm to calculate vector's norm
        slope_vec = float('inf') if vec[0] == 0 else vec[1] / vec[0]
        x_sign_vec, y_sign_vec = np.sign(vec[0]), np.sign(vec[1])
        ...
        for saddle in vecs0:
            ...
            x_sign_saddle, y_sign_saddle = np.sign(saddle[0]), np.sign(saddle[1])
    ...
    # Find the counter-clockwise angle from the x-axis to the eigenvectors
    thetas = [np.arccos(np.dot(np.array([[1, 0]]), saddle_vec) / np.linalg.norm(saddle_vec)) for saddle_vec in saddle_vecs]
    thetas = [2 * math.pi - theta if saddle_vec[1] < 0 else theta for theta, saddle_vec in zip(thetas, saddle_vecs)]

    # Find the rotation matrix that takes the vector (1,0) to the vector in the direction of each eigenvector
    rots = [np.array([[round(np.cos(theta), 5), round(-np.sin(theta), 5)],
                      [round(np.sin(theta), 5), round(np.cos(theta), 5)]]) for theta in thetas]

    # Find a constant value such that mult*rot@(1,0) = saddle_vec while accounting for rounding errors and zero matrix inputs
    mults = []
    for i, rot in enumerate(rots):
        matrix = rot @ np.array([[1], [0]])
        mult1 = saddle_vecs[i][0][0] / matrix[0][0] if matrix[0][0] != 0 else 0
        mult2 = saddle_vecs[i][1][0] / matrix[1][0] if matrix[1][0] != 0 else 0
        if mult1 != 0 and mult2 == 0:
            mult = mult1
        elif mult1 == 0 and mult2 != 0:
            mult = mult2
    ...
    # Find c_inv and c
    Cs = [np.linalg.inv(mult * rot) for mult, rot in zip(mults, rots)]
    C_invs = [mult * rot for mult, rot in zip(mults, rots)]

    # Alpha is the top right value of the matrix M = c @ generator @ c_inv. M must have 1s on the diagonal and 0 in the bottom left
    alphas = []
    Ms = []
    for i, generator in enumerate(generators):
        M = Cs[i] @ generator @ C_invs[i]
        Ms.append(M)
        if not (-1/1000000 < M[1][0] < 1/1000000):
